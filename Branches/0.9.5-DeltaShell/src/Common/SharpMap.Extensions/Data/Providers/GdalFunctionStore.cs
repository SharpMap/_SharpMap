...
namespace SharpMap.Extensions.Data.Providers
{
    ...
    public class GdalFunctionStore : IFunctionStore, IFileBased
    {
        ...
        public virtual string Path
        {
            ...
            set
            {
                 ...
                 * }
                 */
                // do nothing if path stays the same.
                if (path == value)
                {
                    return;
                }
        ...
        public object Clone()
        {
            var clone = new GdalFunctionStore { Path = Path, FireEvents = FireEvents };
            clone.Open();
        ...
        /// </summary>
        public IMultiDimensionalArray GetVariableValues(IVariable variable, IVariableFilter[] filters)
        {
            //Non Gdal Values
            ...
            if (!Grid.Components.Contains(variable))
            {
                throw new NotSupportedException("Variable not part of grid");
            }
        ...
        public IMultiDimensionalArray<T> GetVariableValues<T>(IVariable function, params IVariableFilter[] filters) where T : IComparable
        {
        ...
        public T GetMaxValue<T>(IVariable variable) where T : IComparable
        {
        ...
        public T GetMinValue<T>(IVariable variable) where T : IComparable
        {
            using (var band = gdalDataset.GetRasterBand(1))
            {
                double minimum;
