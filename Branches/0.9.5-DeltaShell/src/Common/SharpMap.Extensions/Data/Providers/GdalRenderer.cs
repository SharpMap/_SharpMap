...
namespace SharpMap.Extensions.Data.Providers
{
    ...
    public class GdalRenderer : RegularGridCoverageRenderer
    {
        ...
        private static ImageSettings GetDefaultImageSettings(Dataset ds, bool dealingWithValuesInsteadOfColors)
        {
            List<OSGeo.GDAL.ColorInterp> interps = GetRasterColorInterps(ds);
            ...
            {
                settings = dealingWithValuesInsteadOfColors ? CreateSettings(PixelFormat.Format32bppArgb, 4) :
                    CreateSettings(PixelFormat.Format16bppRgb555, 2);
            }
            else
            {
        ...
        private void FillBitmapPixels<T>(IList<T> values, BitmapData bitmapData, int targetPixelSpace, int imageWidth, int imageHeight)
        {
            if (layer.Theme == null)
            {
                ...
                layer.Theme = GenerateDefaultTheme(values, layer.Name, new[] { noDataValueObject }, minValue, maxValue);
            }
