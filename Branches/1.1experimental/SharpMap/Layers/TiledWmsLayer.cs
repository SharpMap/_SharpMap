...
namespace SharpMap.Layers
{
    ...
    public class TiledWmsLayer : Layer, IGdiRasterLayer
    {
        #region Fields
        ...
        public Bitmap WmsGetMap(BoundingBox extent, TileSet tileSet)
        {
        ...
        public void DrawToGraphics(Map m, BoundingBox e, Graphics g)
        {
            ...
            try
            {
                foreach (string key in this.TileSetsActive)
                {
                    TileSet tileSet = this.TileSets[key];

                    tileSet.Verify();

                    List<BoundingBox> tileExtents = TileExtents.GetTileExtents(tileSet, m.Envelope, m.PixelSize);
                    ...
                    foreach (BoundingBox tileExtent in tileExtents)
                    {
                        ...
                        {
                            bitmap = this.WmsGetMap(tileExtent, tileSet);
                            if ((tileSet.TileCache != null) && (bitmap != null))
                            {
                                tileSet.TileCache.AddTile(tileExtent, bitmap);
                            }
                        ...
                        if (bitmap != null)
                        {
                            PointF destMin = Transform.WorldtoMap(tileExtent.Min, m);
                            PointF destMax = Transform.WorldtoMap(tileExtent.Max, m);
                            ...
                            g.DrawImage(bitmap, new Rectangle(x, y, width, height),
                              srcRect.Left, srcRect.Top, srcRect.Width, srcRect.Height,
                              GraphicsUnit.Pixel, this._ImageAttributes);
