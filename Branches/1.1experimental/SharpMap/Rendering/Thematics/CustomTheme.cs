...
namespace SharpMap.Rendering.Thematics
{
    ...
    public delegate IStyle GetStyleMethod(SharpMap.Data.FeatureDataRow dr);


    public delegate TStyle GetStyleMethod<TStyle>(FeatureDataRow fdr) where TStyle : IStyle;
    ...
    public class CustomTheme<TStyle>
        : CustomTheme, ITheme<TStyle> where TStyle : IStyle
    {
        public CustomTheme(GetStyleMethod<TStyle> getStyleMethod)
            : base()
        {
            _getStyleDelegate = getStyleMethod;
        }
        ...
        /// Gets or sets the default style when an attribute isn't found in any bucket
        /// </summary>
        public new TStyle DefaultStyle
        {
            get { return _DefaultStyle; }
            set { _DefaultStyle = value; }
        }

        private GetStyleMethod<TStyle> _getStyleDelegate;

        #region ITheme<TStyle> Members

        public new TStyle GetStyle(SharpMap.Data.FeatureDataRow attribute)
        {
            TStyle s = _getStyleDelegate(attribute);
            s = s != null ? s : _DefaultStyle;
            return s;
        }
