	...
	public class GeometryFromWKT
	{
		...
		*/
		...
		private static Polygon ReadPolygonText(WktStreamTokenizer tokenizer)
		{
			...
			//if (pol.Vertices[0] != pol.Vertices[pol.Vertices.Count - 1])
			//    pol.Vertices.Add(pol.Vertices[0]);
			nextToken = GetNextCloserOrComma(tokenizer);
			while (nextToken == ",")
			{
				//Add holes
				//Polygon polInterior = new Polygon();	
				pol.InteriorRings.Add(new LinearRing(GetCoordinates(tokenizer)));
				//if (polInterior.Vertices[0] != polInterior.Vertices[polInterior.Vertices.Count - 1])
				//    polInterior.Vertices.Add(polInterior.Vertices[0]);
				nextToken = GetNextCloserOrComma(tokenizer);
			}
			return pol;
