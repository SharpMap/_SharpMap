...
namespace SharpMap.Converters.WellKnownText
{
	...
	internal class WktStreamTokenizer : StreamTokenizer
	{
		...
		public void ReadAuthority(ref string authority,ref long authorityCode)
		{
			...
			ReadToken(",");
#if !CFBuild //TryParse not supported in CF. I think Convert will do the same thing. This can likely be changed in the full framework version with no ill effect.
			long.TryParse(this.ReadDoubleQuotedWord(),out authorityCode);			
#else
            authorityCode = Convert.ToInt64(this.ReadDoubleQuotedWord());
#endif
			ReadToken("]");
		}
		#endregion
