...
namespace SharpMap.CoordinateSystems.Transformations
{
	...
	public class GeometryTransform
	{
		...
		public static BoundingBox TransformBox(BoundingBox box, IMathTransform transform)
		{
			...
			BoundingBox result = new BoundingBox(corners[0],corners[0]);
			for (int i = 1; i < 4; i++)
			{
				result.Min.X = Math.Min(result.Min.X, corners[i].X);
				result.Min.Y = Math.Min(result.Min.Y, corners[i].Y);
				result.Max.X = Math.Max(result.Max.X, corners[i].X);
				result.Max.Y = Math.Max(result.Max.Y, corners[i].Y);
			}
			return result;
		}
		/// <summary>
		...
		public static Polygon TransformPolygon(Polygon p, IMathTransform transform)
		{
			...
			pOut.ExteriorRing = TransformLinearRing(p.ExteriorRing, transform);
			...
			for (int i = 0; i < p.InteriorRings.Count; i++)
				p.InteriorRings.Add(TransformLinearRing(p.InteriorRings[i], transform));
			return pOut;
		}
		/// <summary>
