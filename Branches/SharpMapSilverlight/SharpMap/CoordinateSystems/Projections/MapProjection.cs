...
namespace SharpMap.CoordinateSystems.Projections
{
	...
	internal abstract class MapProjection : MathTransform, IProjection
	{
		...
		protected MapProjection(List<ProjectionParameter> parameters)
		{
			...
			this._semiMajor = _Parameters.Find(delegate(ProjectionParameter par)
								{ return par.Name.Equals("semi_major", StringComparison.OrdinalIgnoreCase); }).Value;
			this._semiMinor = _Parameters.Find(delegate(ProjectionParameter par)
...
#if !CFBuild //Different overload in CF, Almost the same. 
			throw new ArgumentOutOfRangeException("x",x," not a valid longitude in degrees.");
#else
            throw new ArgumentOutOfRangeException("x", " not a valid longitude in degrees.");
#endif
		}
...
#else
            throw new ArgumentOutOfRangeException("x", " not a valid latitude in degrees.");
#endif
