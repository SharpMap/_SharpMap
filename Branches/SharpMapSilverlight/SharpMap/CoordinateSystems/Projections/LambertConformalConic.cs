...
namespace SharpMap.CoordinateSystems.Projections
{
	...
	internal class LambertConformalConic2SP : MapProjection
	{
			...
			: base(parameters, isInverse)
		{
			...
			ProjectionParameter latitude_of_origin = _Parameters.Find(delegate(ProjectionParameter par) { return par.Name.Equals("latitude_of_origin", StringComparison.OrdinalIgnoreCase); });
			ProjectionParameter central_meridian = _Parameters.Find(delegate(ProjectionParameter par) { return par.Name.Equals("central_meridian", StringComparison.OrdinalIgnoreCase); });
			ProjectionParameter standard_parallel_1 = _Parameters.Find(delegate(ProjectionParameter par) { return par.Name.Equals("standard_parallel_1", StringComparison.OrdinalIgnoreCase); });
			ProjectionParameter standard_parallel_2 = _Parameters.Find(delegate(ProjectionParameter par) { return par.Name.Equals("standard_parallel_2", StringComparison.OrdinalIgnoreCase); });
			ProjectionParameter false_easting = _Parameters.Find(delegate(ProjectionParameter par) { return par.Name.Equals("false_easting", StringComparison.OrdinalIgnoreCase); });
			ProjectionParameter false_northing = _Parameters.Find(delegate(ProjectionParameter par) { return par.Name.Equals("false_northing", StringComparison.OrdinalIgnoreCase); });
