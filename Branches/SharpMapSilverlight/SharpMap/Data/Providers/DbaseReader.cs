        ...
        */
...
#if !CFBuild //CF doesn't have this overload. I believe this is identical, and can be changed in the full framework version.
                DbaseColumns[i].ColumnName = System.Text.Encoding.UTF7.GetString((br.ReadBytes(11))).Replace("\0", "").Trim();                
#else
                DbaseColumns[i].ColumnName = System.Text.Encoding.UTF7.GetString((br.ReadBytes(11)),0,11).Replace("\0", "").Trim();                
#endif
        ...
        private System.Text.Encoding GetDbaseLanguageDriver(byte dbasecode)
        {
            switch (dbasecode)
            {
                ...
                case 0x64: return System.Text.Encoding.GetEncoding(852); // EE MS-DOS code page 852 
                case 0x65: return System.Text.Encoding.GetEncoding(865); // Nordic MS-DOS code page 865 
                case 0x66: return System.Text.Encoding.GetEncoding(866); // Russian MS-DOS code page 866 
                ...
                case 0xC8: return System.Text.Encoding.GetEncoding(1250); // Windows EE code page 1250 
                ...
                case 0x7A: return System.Text.Encoding.GetEncoding(936); // Chinese (PRC, Singapore) Windows 
                case 0x7B: return System.Text.Encoding.GetEncoding(932); // Japanese Windows 
                case 0x7C: return System.Text.Encoding.GetEncoding(874); // Thai Windows 
                ...
                case 0x79: return System.Text.Encoding.GetEncoding(949); // Korean Windows 
                case 0x78: return System.Text.Encoding.GetEncoding(950); // Chinese (Hong Kong SAR, Taiwan) Windows
                case 0x00: return System.Text.Encoding.UTF7; 
                default:
        ...
        private SharpMap.Data.FeatureTable baseTable;

        private void CreateBaseTable()
        {
            baseTable = new SharpMap.Data.FeatureTable();
            //!!!
            //foreach (DbaseField dbf in DbaseColumns)
        ...
        /// <returns></returns>
        internal IFeatureRow GetFeature(uint oid, IFeatureTable table)
        {
            if (!_isOpen)
                throw (new ApplicationException("An attempt was made to read from a closed DBF file"));
            if (oid >= _NumberOfRecords)
        ...
        private object ReadDbfValue(DbaseField dbf)
        {
            switch (dbf.DataType.ToString())
            {
                case "System.String":
                    if (_Encoding == null)
#if !CFBuild //Simple Getstring overload not in CF, use the more general one. I believe this is identical, and can be changed in the full framework version.
...
#else
                    {
                        return _FileEncoding.GetString(br.ReadBytes(dbf.Length), 0, dbf.Length).Replace("\0", "").Trim();
                    }
                    else
                    {
                        return _Encoding.GetString(br.ReadBytes(dbf.Length), 0, dbf.Length).Replace("\0", "").Trim();
                    }
#endif
                case "System.Double":
#if !CFBuild //Simple Getstring overload not in CF, use the more general one. I believe this is identical, and can be changed in the full framework version.
...
#else
                    string temp = System.Text.Encoding.UTF7.GetString(br.ReadBytes(dbf.Length),0,dbf.Length).Replace("\0", "").Trim();
#endif
#if !CFBuild //TryParse not in CF - use Convert and a try catch to do the same thing. I believe this is identical, and can be changed in the full framework version.
                    double dbl = 0;
...
#else
                    string temp16 = System.Text.Encoding.UTF7.GetString((br.ReadBytes(dbf.Length)),0,dbf.Length).Replace("\0", "").Trim();
#endif
...
#else
                    string temp32 = System.Text.Encoding.UTF7.GetString((br.ReadBytes(dbf.Length)),0,dbf.Length).Replace("\0", "").Trim();
#endif
...
#else
                    string temp64 = System.Text.Encoding.UTF7.GetString((br.ReadBytes(dbf.Length)),0,dbf.Length).Replace("\0", "").Trim();
#endif
...
#else
                    string temp4 = System.Text.Encoding.UTF8.GetString((br.ReadBytes(dbf.Length)),0,dbf.Length);
#endif
                ...
                case "System.DateTime":
...
#if !CFBuild //TryParseExact not in CF, GetString to supported overload. I believe this is identical, and can be changed in the full framework version.
                    DateTime date;
                    if (DateTime.TryParseExact(System.Text.Encoding.UTF7.GetString((br.ReadBytes(8))),
                        ...
                        return DBNull.Value;
#else
                    string temp5 = System.Text.Encoding.UTF7.GetString((br.ReadBytes(8)), 0, 8);
                    try
                    {
                        return Convert.ToDateTime(temp5);
                    }
                    catch
                    {
                        return DBNull.Value;
