...
namespace SharpMap.Geometries
{
    ...
    public class MultiLineString : MultiCurve
	{
		...
		{
			if (this.LineStrings.Count == 0)
				return null;
			BoundingBox bbox = this.LineStrings[0].GetBoundingBox();
			for (int i = 1; i < this.LineStrings.Count; i++)
				bbox = bbox.Join(this.LineStrings[i].GetBoundingBox());
			return bbox;
		}
