	...
	public class QuadTree : IDisposable
	{
		...
		public static QuadTree FromFile(string filename)
		{
			...
			if (br.ReadDouble() != INDEXFILEVERSION) //Check fileindex version
				throw (new Exception("Invalid index file version. Please rebuild the spatial index by either deleting the index or calling SharpMap.Data.Providers.ShapeFile.RebuildSpatialIndex()"));
