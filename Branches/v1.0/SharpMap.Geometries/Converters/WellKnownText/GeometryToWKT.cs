...
namespace SharpMap.Converters.WellKnownText
{
    ...
    public class GeometryToWKT
    {
        ...
        private static void AppendPointText(Point coordinate, StringWriter writer)
        {
            ...
            {
                writer.Write("(");
                AppendCoordinate(((IPoint)coordinate).Coordinate, writer);
                writer.Write(")");
            }
        }
        ...
        private static void AppendPolygonText(Polygon polygon, StringWriter writer)
        {
            ...
            {
                writer.Write("(");
                AppendLineStringText((LinearRing)polygon.ExteriorRing, writer);
                for (int i = 0; i < polygon.InteriorRings.Length; i++)
                {
                    writer.Write(", ");
                    AppendLineStringText((LinearRing)polygon.InteriorRings[i], writer);
                }
                writer.Write(")");
            }
        }
        ...
        private static void AppendMultiPointText(MultiPoint multiPoint, StringWriter writer)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < multiPoint.Points.Count; i++)
                {
                    if (i > 0)
                        writer.Write(", ");
                    AppendCoordinate(((IPoint)multiPoint[i]).Coordinate, writer);
                }
                writer.Write(")");
            }
        }
