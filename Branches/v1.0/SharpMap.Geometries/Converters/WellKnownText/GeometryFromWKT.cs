...
namespace SharpMap.Converters.WellKnownText
{
    ...
    public class GeometryFromWKT
    {
        ...
        private static Collection<Coordinate> GetCoordinates(WktStreamTokenizer tokenizer)
        {
            ...
            var externalCoordinate = new Coordinate {X = GetNextNumber(tokenizer), Y = GetNextNumber(tokenizer)};
            coordinates.Add(externalCoordinate);
            nextToken = GetNextCloserOrComma(tokenizer);
            while (nextToken == ",")
            {
                var internalCoordinate = new Coordinate {X = GetNextNumber(tokenizer), Y = GetNextNumber(tokenizer)};
        ...
        private static Geometry ReadGeometryTaggedText(WktStreamTokenizer tokenizer)
        {
            ...
            switch (type)
            {
                ...
                case "GEOMETRYCOLLECTION":
                    ...
                    break;
                default:
                    throw new Exception(String.Format(System.Globalization.CultureInfo.InvariantCulture, "Geometrytype '{0}' is not supported.",
                                                      type));
            }
            return geometry;
        }
        ...
        private static Polygon ReadPolygonText(WktStreamTokenizer tokenizer)
        {
            ...
            nextToken = GetNextCloserOrComma(tokenizer);
            var interiorRings = new List<ILineString>();
            while (nextToken == ",")
            {
                //Add holes
                interiorRings.Add(new LinearRing(GetCoordinates(tokenizer)));
                nextToken = GetNextCloserOrComma(tokenizer);
            }
            pol.InteriorRings = interiorRings.ToArray();
            return pol;
        }
