...
namespace SharpMap.Data.Providers
{
    ...
    {
        private readonly IBinaryGeometryReader _reader;
        ...
        public int SRID
        {
            ...
            {
                _srid = value;
                _reader.Factory = GeometryServices.Instance[value];
            }
        }
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (dr["sharpmap_tempgeometry"] != DBNull.Value)
                                fdr.Geometry = _reader.Read((byte[]) dr["sharpmap_tempgeometry"]);
                            return fdr;
                        }
                        else
                            return null;
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(strSQL, conn))
                {
                    ...
                    using (SqlDataReader dr = command.ExecuteReader())
                        if (dr.Read())
                        {
                            box = new GeoAPI.Geometries.Envelope((float) dr[0], (float) dr[1], (float) dr[2], (float) dr[3]);
                        }
                    conn.Close();
                }
                return box;
